...
def iterate():
    for tol, rollList in itertools.product(range(0,10),range(1, len(roll_list)+1)):
        for seq in itertools.combinations(roll_list, rollList):    
            if sum(seq)==target_number+tol or sum(seq) == target_number:
                exitos.append(list(seq))
                for element in seq:
                    if element in roll_list:
                        roll_list.remove(element)
